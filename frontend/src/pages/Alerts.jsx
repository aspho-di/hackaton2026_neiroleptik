...
export default function Alerts() {
    ...
    useEffect(() => {
        fetchAlerts().then(data => {
            if (data && Array.isArray(data)) setAlertsList(data)
        })
    }, [])
    ...
    function markAllRead() {
        const all = alertsList.map(a => a.id)
        setReadIds(all)
        localStorage.setItem('alerts_read', JSON.stringify(all))
    }

    const filtered = alertsList.filter(a => filter === 'all' || a.type === filter)
    const unread   = alertsList.filter(a => !readIds.includes(a.id)).length
